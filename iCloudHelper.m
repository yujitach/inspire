...
static NSURL*ubiquityContainer;
...
+(void)setupWithUbiquityContainerIdentifier:(NSString*)container completion:(void(^)(NSURL*ubiquityContainerURL))handler
{
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ubiquityContainer=[[NSFileManager defaultManager] URLForUbiquityContainerIdentifier:container];
        handler(ubiquityContainer);
    });
}
+(NSMetadataQuery*)metadataQueryForExtension:(NSString*)extension
{
    ...
    query.predicate=[NSPredicate predicateWithFormat:[NSString stringWithFormat:@"%%K.pathExtension LIKE '%@'", extension], NSMetadataItemFSNameKey];
    return query;
}
...
+ (void)saveAndCloseDocumentWithName:(NSString *)documentName withContent:(NSData *)content completion:(void (^)(BOOL success))handler {
    ...
    UIDocumentSaveOperation op=[[NSFileManager defaultManager] fileExistsAtPath:[fileURL path]]?UIDocumentSaveForOverwriting:UIDocumentSaveForCreating;
    [document saveToURL:document.fileURL forSaveOperation:op completionHandler:^(BOOL success) {
        if (success) {
    ...
    if(![status isEqualToString:NSURLUbiquitousItemDownloadingStatusCurrent]){
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:[fileURL path]]) {
            ...
            if (document.documentState & UIDocumentStateClosed) {
                ...
                handler(document.contents); //UIDocumentStateNormal
            }
        } else {
            handler(nil);
        }
